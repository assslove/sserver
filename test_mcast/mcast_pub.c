 ...
 * =====================================================================================
 *
 *       Filename:  mcast_pub.c
 *
 *    Description:  mcast公共接口定义处 需要与mcast.h保持一致
 *
 *        Version:  1.0
 *        Created:  2015年03月18日 20时00分57秒
 *       Revision:  none
 *       Compiler:  gcc
 *
...
int mcast_cli_init(char *mcast_ip, uint16_t mcast_port, char *local_ip) 
{
	...
	if (sockfd == -1) {
		fprintf(0, "failed create mcast fd [%s]", strerror(errno));
		return -1;
	}
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(flag)) == -1) {
		fprintf(0, "set sock reuseaddr failed [%s]", strerror(errno));
		return -1;
	}

	int ret = bind(sockfd, (struct sockaddr*)&local_sa, sizeof(struct sockaddr_in));
	if (ret == -1) {
		fprintf(0, "mcast bind failed [%s]", strerror(errno));
		return -1;
	}

	if (set_mcast_ttl(sockfd, 1) == -1) {
		fprintf(0, "set mcast ttl failed [%s]", strerror(errno));
		return -1;
	}

	if (set_mcast_loop(sockfd, 1) == -1) {
		fprintf(0, "set mcast loop failed [%s]", strerror(errno));
		return -1;
	}
	...
	if (join_mcast(sockfd, &req) == -1) {
		fprintf(0, "join mcast failed [%s]", strerror(errno));
		return -1;
	}
...
int send_pkg_to_mcast(char *mcast_ip, uint16_t mcast_port, char *local_ip, int mcast_type, int len, void *data)
{
	int sockfd = socket(AF_INET, SOCK_DGRAM, 0);
	if (sockfd == -1) {
		fprintf(0, "create serv socket failed [%s]", strerror(errno));
		return -1;
	}
	...
	if (set_mcast_if(sockfd, local_addr) == -1) {
		fprintf(0, "set mcast if failed [%s]", strerror(errno));
		return -1;
	}
	...
	if (size != pkg->len) {
		fprintf(0, "send error[sendsize=%u][realsize=%u]", size, pkg->len);
		return -1;
	}
